...
interface Issue {
  ...
  vendor: { _id: string, vendorName: string, email: string };
...
const AdminIssues = () => {
  ...
  const fetchIssues = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/issues/admin`, {
  ...
  return (
    <div className="p-8 animate-fade-in">
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-gray-800">Global Product Issues</h2>
        <p className="text-gray-600 mt-1">Monitor platform-wide reports and actions</p>
      </div>

      <div className="bg-white rounded-xl shadow-md overflow-hidden">
        {issues.length === 0 ? (
           <div className="p-10 text-center text-gray-500">
             No issues reported across the platform.
           </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  <th className="p-4 font-semibold text-gray-600 text-sm">Product / Vendor</th>
                  ...
                  <tr key={issue._id} className="hover:bg-gray-50 transition-colors">
                    <td className="p-4">
                      <div className="flex items-center space-x-3">
                        ...
                        />
                        <div>
                          <p className="font-medium text-gray-800 line-clamp-1 max-w-[200px]">{issue.product.title}</p>
                          <p className="text-xs text-blue-600 mt-1">Vendor: {issue.vendor?.vendorName || 'Unknown'}</p>
                        </div>
                      </div>
                    </td>
